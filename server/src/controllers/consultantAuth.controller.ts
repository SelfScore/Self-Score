...
export class ConsultantAuthController {
    ...
    static async registerStep1(req: Request, res: Response): Promise<void> {
        ...
        try {
            ...
            if (!firstName || !lastName || !email || !password || !countryCode || !phoneNumber || !location) {
                const response: ApiResponse = {
                    success: false,
            ...
            const existingConsultant = await ConsultantModel.findOne({ email, isVerified: true });
            ...
            if (!emailSent) {
                console.warn('⚠️  Failed to send verification email, but consultant was created');
            }
    ...
    static async verifyEmail(req: Request, res: Response): Promise<void> {
        try {
            ...
            const response: ApiResponse = {
                success: true,
                message: "Email verified successfully. You can now continue with your registration.",
                data: {
     ...
     */
    static async updateCertifications(req: Request, res: Response): Promise<void> {
        try {
    ...
    static async login(req: Request, res: Response): Promise<void> {
        try {
            ...
            const isPasswordCorrect = await bcrypt.compare(password, consultant.password);
     ...
     */
    static async getCurrentConsultant(req: Request, res: Response): Promise<void> {
        try {
            ...
            const consultant = await ConsultantModel.findById(consultantId).select('-password -verifyCode');
