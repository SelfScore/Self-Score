 ...
 */
export const getLevel4SubmissionDetails = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const existingReview = await Level4ReviewModel.findOne({ interviewId })
            .lean();

        // Prepare question-answer pairs with proper mode tracking
        const questionAnswers = interview.questions.map((q: any) => {
            // Find text answer
            const textAnswer = interview.answers?.find((a: any) => a.questionId === q.questionId);
            
            // Find voice answer from transcript using questionId
            let voiceAnswer = '';
            if (interview.transcript && interview.transcript.length > 0) {
                const voiceResponse = interview.transcript.find((t: any) => 
                    t.role === 'user' && t.questionId === q.questionId
                );
                if (voiceResponse) {
 ...
 */
export const saveDraftReview = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!questionReviews || !Array.isArray(questionReviews) || questionReviews.length !== 8) {
            res.status(400).json({
                success: false,
        ...
        const totalScore = questionReviews.reduce((sum: number, qr: any) => sum + (qr.score || 0), 0);
 ...
 */
export const submitFinalReview = async (req: Request, res: Response): Promise<void> => {
    try {
        const adminId = req.admin?.adminId;
        const { interviewId } = req.params;
        const { questionReviews } = req.body;
        ...
        const invalidReviews = questionReviews.filter((qr: any) => 
        ...
        // Calculate total score
        const totalScore = questionReviews.reduce((sum: number, qr: any) => sum + qr.score, 0);
        ...
        try {
            ...
            if (user) {
                ...
                user.progress.highestUnlockedLevel = Math.max(user.progress.highestUnlockedLevel, 4);
                ...
                console.log(`User ${interview.userId} Level 4 review completed with score ${clampedTotalScore}`);
            }
        } catch (error: any) {
        ...
        try {
            ...
            if (userEmail) {
                await sendLevel4ReviewCompleteEmail(userEmail, userName, clampedTotalScore);
                console.log(`Email sent to ${userEmail} about Level 4 review completion`);
            }
        } catch (error: any) {
 ...
 */
export const getReviewByInterviewId = async (req: Request, res: Response): Promise<void> => {
    try {
