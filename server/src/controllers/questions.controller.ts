...
export const getQuestionsByLevel = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        if (userId) {
            ...
            const questionsWithResponses = questions.map(question => {
                const response = responses.find(r => r.questionId.toString() === (question._id as string).toString());
                return {
                    ...question.toObject(),
...
export const getQuestionsWithResponses = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        if (!questions.length) {
            return res.status(404).json({
                success: false,
                message: level ? `No questions found for level ${level}` : "No questions found",
            });
        }
