...
const REDIRECT_URI = process.env.GOOGLE_CALENDAR_REDIRECT_URI || 'http://localhost:3000/consultant/calendar/callback';
...
export class GoogleCalendarService {
    ...
    constructor() {
        if (!CLIENT_ID || !CLIENT_SECRET) {
            console.warn('⚠️  Google Calendar credentials not configured. Calendar features will not work.');
        }
    ...
    async createEvent(
        ...
        return {
            eventId: data.id,
            meetingLink: data.hangoutLink || data.conferenceData?.entryPoints?.[0]?.uri || undefined,
