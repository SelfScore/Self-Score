...
export const bookingService = {
  ...
  ): Promise<ApiResponse<{ slots: TimeSlot[]; count: number }>> => {
    try {
      ...
      const response = await api.get(
        `/api/booking/availability/${params.consultantId}?${queryParams}`
      );
  ...
  confirmBooking: async (
    ...
    try {
      const response = await api.post(
        `/api/booking/${params.bookingId}/confirm`,
        {
  ...
  cancelBooking: async (params: CancelBookingParams): Promise<ApiResponse> => {
    try {
      const response = await api.post(
        `/api/booking/${params.bookingId}/cancel`,
        {
  ...
  getConsultantBookings: async (params?: {
    status?: string;
    upcoming?: boolean;
  }): Promise<ApiResponse<Booking[]>> => {
    try {
      ...
      const query = queryParams.toString() ? `?${queryParams}` : "";
      const response = await api.get(
        `/api/booking/consultant/my-bookings${query}`
      );
      return response as unknown as ApiResponse<Booking[]>;
    } catch (error) {
  ...
  // Cancel a booking as consultant
  consultantCancelBooking: async (
    params: CancelBookingParams
  ): Promise<ApiResponse> => {
    try {
      const response = await api.post(
        `/api/booking/consultant/${params.bookingId}/cancel`,
        {
          cancellationReason: params.cancellationReason,
        }
      );
